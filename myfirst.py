...
def get_sub_video(args, num_process, process_idx):
    ...
    cmd = [
    args.ffmpeg_bin, '-hwaccel cuda', '-i', f'{args.input}', '-ss', f'{part_time * process_idx}',
    f'-to {part_time * (process_idx + 1)}' if process_idx != num_process - 1 else '',
    '-c:v', 'h264_nvenc', '-preset', 'fast', '-async', '1', out_path, '-y'
    ]
...
class Writer:
    ...
    .output(
        audio if audio is not None else 'anullsrc',
        video_save_path,
        pix_fmt='yuv420p',
        vcodec='h264_nvenc',  # Use GPU encoding
        preset='fast',
        loglevel='error',
        acodec='copy' if audio else 'aac'
    )
    .overwrite_output()
...
def inference_video(args, video_save_path, device="cuda", total_workers=1, worker_idx=0):
...
def run(args):
    ...
    cmd = [
    args.ffmpeg_bin, '-hwaccel cuda', '-f', 'concat', '-safe', '0', '-i',
    f'{args.output}/{args.video_name}_vidlist.txt', '-c:v', 'h264_nvenc', '-preset', 'fast', '-c:a', 'aac',
    f'{video_save_path}'
]
